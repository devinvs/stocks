...
fn update_stock_info(info: &mut HashMap<String, (f64, f64)>) {
    for (symbol, i) in info.iter_mut() {
        *i = get_nasdaq_value(symbol, "stocks").unwrap_or_else(|| {
            get_nasdaq_value(symbol, "etf").unwrap_or_default()
        });
    }
}
...
fn get_nasdaq_value(symbol: &str, class: &str) -> Option<(f64, f64)> {
        ...
        let url = format!("https://api.nasdaq.com/api/quote/{}/info?assetclass={}", symbol, class);
